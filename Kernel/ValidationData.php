...
class ValidationData
{
    ...
    const DEFAULT_ARRAY_MSG      = "Field ':field:' must be array";
    ...
    protected function apply($field, $rule, $params)
    {
        switch ($rule) {
            ...
            case "regexp":
                ...
                break;
            case "array" :
                $this->checkArray($field, $params['message'] ?? null);
        }
    }
     ...
     * @param $message
     */
    protected function checkArray($field, $message)
    {
        if (isset($this->fields[$field]) && (!is_array($this->fields[$field]))) {
            $message = $this->setMessage($message ?? self::DEFAULT_ARRAY_MSG, [':field:' => $field]);
